...
def runGraph():
    height = [svm_acc,ann_acc,decision_acc,ensemble_acc]
    bars = ('SVM Accuracy','ANN Accuracy','Decision Tree Accuracy','Ensemble Accuracy')
...
font = ('times', 18, 'bold')
title = Label(root, text='Personalized Diabetes Suggestion Using ML Algorithms and Cloud Technology')
...
treeButton = Button(root, text="Run SVM Algorithm", command=runSVM)
treeButton.place(x=50,y=200)
treeButton.config(font=font1)

svmButton = Button(root, text="Run ANN Algorithm", command=runANN)
svmButton.place(x=50,y=250)
svmButton.config(font=font1)

annButton = Button(root, text="Run Decision Tree Algorithm", command=decisionTree)
...
graphs = Button(root, text="Graph Analyzing", command=runGraph)
